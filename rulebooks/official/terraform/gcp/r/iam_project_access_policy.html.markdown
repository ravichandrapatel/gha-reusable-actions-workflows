...
* `id` - an identifier for the resource with format `projects/{{project}}/locations/{{location}}/accessPolicies/{{access_policy_id}}`
...
import {
  ...
  to = google_iam_project_access_policy.default
}
```

In Terraform v1.5.0 and later, use an [`import` block](https://developer.hashicorp.com/terraform/language/import) to import ProjectAccessPolicy using one of the formats above. For example:

```tf
import {
  id = "projects/{{project}}/locations/{{location}}/accessPolicies/{{access_policy_id}}"
  to = google_iam_project_access_policy.default
}
```

When using the [`terraform import` command](https://developer.hashicorp.com/terraform/cli/commands/import), ProjectAccessPolicy can be imported using one of the formats above. For example:

```
$ terraform import google_iam_project_access_policy.default projects/{{project}}/locations/{{location}}/accessPolicies/{{access_policy_id}}
$ terraform import google_iam_project_access_policy.default {{project}}/{{location}}/{{access_policy_id}}
$ terraform import google_iam_project_access_policy.default {{location}}/{{access_policy_id}}
```
