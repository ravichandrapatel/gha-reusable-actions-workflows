...
<a name="nested_assignment_group_labels"></a>The `group_labels` block supports:

* `labels` -
  (Required)
  Google Compute Engine instance labels that must be present for an instance to be included in this assignment group.

<a name="nested_assignment_os_types"></a>The `os_types` block supports:
...
In Terraform v1.12.0 and later, use an [`identity` block](https://developer.hashicorp.com/terraform/language/block/import#identity) to import GuestPolicies using identity values. For example:

```tf
import {
  identity = {
    guestPolicyId = "<-required value->"
