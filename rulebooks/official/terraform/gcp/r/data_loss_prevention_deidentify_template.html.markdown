...
<a name="nested_deidentify_config_record_transformations_record_suppressions"></a>The `record_suppressions` block supports:
...
In Terraform v1.12.0 and later, use an [`identity` block](https://developer.hashicorp.com/terraform/language/block/import#identity) to import DeidentifyTemplate using identity values. For example:
...
When using the [`terraform import` command](https://developer.hashicorp.com/terraform/cli/commands/import), DeidentifyTemplate can be imported using one of the formats above. For example:
