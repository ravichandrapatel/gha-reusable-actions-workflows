...
```hcl
data "google_storage_control_project_intelligence_finding_revisions" "revisions" {
...
* `page_size` - (Optional) The maximum number of IntelligenceFindingRevision resources to return.

## Attributes Reference

The following attributes are exported:

* `id` - an identifier for the resource with format `projects/{{project}}/locations/{{location}}/intelligenceFindings/{{finding_id}}/revisions`

* `revisions` - The list of intelligence finding revisions. Structure is [documented below](#nested_revisions).

<a name="nested_revisions"></a>The `revisions` block contains:
