...
* `id` - an identifier for the resource with format `organizations/{{organization}}/locations/{{location}}/accessPolicies/{{access_policy_id}}`
...
In Terraform v1.12.0 and later, use an [`identity` block](https://developer.hashicorp.com/terraform/language/block/import#identity) to import OrganizationAccessPolicy using identity values. For example:
...
import {
  ...
  to = google_iam_organization_access_policy.default
}
```

In Terraform v1.5.0 and later, use an [`import` block](https://developer.hashicorp.com/terraform/language/import) to import OrganizationAccessPolicy using one of the formats above. For example:

```tf
import {
  id = "organizations/{{organization}}/locations/{{location}}/accessPolicies/{{access_policy_id}}"
  to = google_iam_organization_access_policy.default
}
```

When using the [`terraform import` command](https://developer.hashicorp.com/terraform/cli/commands/import), OrganizationAccessPolicy can be imported using one of the formats above. For example:

```
$ terraform import google_iam_organization_access_policy.default organizations/{{organization}}/locations/{{location}}/accessPolicies/{{access_policy_id}}
$ terraform import google_iam_organization_access_policy.default {{organization}}/{{location}}/{{access_policy_id}}
```
