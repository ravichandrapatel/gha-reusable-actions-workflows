...
<div class = "oics-button" style="float: right; margin: 0 0 -15px">
  <a href="https://console.cloud.google.com/cloudshell/open?cloudshell_git_repo=https%3A%2F%2Fgithub.com%2Fterraform-google-modules%2Fdocs-examples.git&cloudshell_image=gcr.io%2Fcloudshell-images%2Fcloudshell%3Alatest&cloudshell_print=.%2Fmotd&cloudshell_tutorial=.%2Ftutorial.md&cloudshell_working_dir=migration_center_report_config_basic&open_in_editor=main.tf" target="_blank">
...
* `group_preferenceset_assignments` -
  ...
  Structure is [documented below](#nested_group_preferenceset_assignments).
...
<a name="nested_group_preferenceset_assignments"></a>The `group_preferenceset_assignments` block supports:
...
* `id` - an identifier for the resource with format `projects/{{project}}/locations/{{location}}/reportConfigs/{{report_config_id}}`
...
In Terraform v1.12.0 and later, use an [`identity` block](https://developer.hashicorp.com/terraform/language/block/import#identity) to import ReportConfig using identity values. For example:

```tf
import {
  identity = {
    location = "<-required value->"
    reportConfigId = "<-required value->"
    project = "<-optional value->"
  }
  to = google_migration_center_report_config.default
}
```
...
```tf
import {
  id = "projects/{{project}}/locations/{{location}}/reportConfigs/{{report_config_id}}"
  to = google_migration_center_report_config.default
}
```
...
```
$ terraform import google_migration_center_report_config.default projects/{{project}}/locations/{{location}}/reportConfigs/{{report_config_id}}
$ terraform import google_migration_center_report_config.default {{project}}/{{location}}/{{report_config_id}}
$ terraform import google_migration_center_report_config.default {{location}}/{{report_config_id}}
```
