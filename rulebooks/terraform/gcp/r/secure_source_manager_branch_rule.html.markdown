...
In Terraform v1.12.0 and later, use an [`identity` block](https://developer.hashicorp.com/terraform/language/block/import#identity) to import BranchRule using identity values. For example:

```tf
import {
  identity = {
    branch_rule_id = "<-required value->"
