...
* `document` -
  ...
  Structure is [documented below](#nested_openapi_documents_document).


<a name="nested_openapi_documents_document"></a>The `document` block supports:
...
* `file_descriptor_set` -
  ...
  Structure is [documented below](#nested_grpc_services_file_descriptor_set).
...
* `source` -
  ...
  Structure is [documented below](#nested_grpc_services_source).


<a name="nested_grpc_services_file_descriptor_set"></a>The `file_descriptor_set` block supports:
...
<a name="nested_grpc_services_source"></a>The `source` block supports:
...
In Terraform v1.12.0 and later, use an [`identity` block](https://developer.hashicorp.com/terraform/language/block/import#identity) to import ApiConfig using identity values. For example:
