...
In Terraform v1.12.0 and later, use an [`identity` block](https://developer.hashicorp.com/terraform/language/block/import#identity) to import FirewallPolicyWithRules using identity values. For example:

```tf
import {
  identity = {
    policyId = "<-optional value->"
  }
