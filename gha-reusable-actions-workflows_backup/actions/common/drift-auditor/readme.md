...
- **Purpose**: Detect and report Terraform drift (and zombie workspaces) across many workspaces with one run.
- **Scope**: Composite action + Python script; parallelizes `terraform plan` safely without a workflow matrix.
- **Primary users**: Platform/DevOps teams managing many Terraform workspaces with a shared backend.
- **Success criteria**: Drift report is generated and (optionally) a single repo issue is created/updated/closed to reflect current drift state.
...
| **Repository / Code** | `devtools-landingzone/actions/drift-auditor` |
| **Dependencies** | Terraform CLI, backend credentials (e.g. AWS OIDC), GitHub REST API (Issues) |
