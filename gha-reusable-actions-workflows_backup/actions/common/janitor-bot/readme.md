...
- **Purpose**: Reduce GitHub repo/org hygiene toil by scanning and optionally cleaning up stale branches/PRs/artifacts/packages.
- **Scope**: Composite action + Python script; can target a repo, multiple repos, an org, or topic-filtered repos.
- **Primary users**: Platform/DevOps engineers maintaining large GitHub orgs.
- **Success criteria**: A `janitor_report.md` is produced (scan) and/or requested cleanup operations complete without touching protected branches.
...
| **Repository / Code** | `devtools-landingzone/actions/janitor-bot` |
| **Dependencies** | GitHub REST API, Python (stdlib) |
