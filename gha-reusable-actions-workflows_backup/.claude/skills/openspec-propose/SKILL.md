...
5. **Missing pins/recipes:** after pack, if cards lack required checkout/artifact/house-action facts → corpus scan → live Git/OCI → **MUST write-back**. **FORBIDDEN** at plan time: approving stub `run:` / local staging / “promote later” as the design when the task needs real cross-job artifacts or `./actions/*` composites.
...
3. **Grill-me (BINDING — before design/specs)**
   ...
   Follow the `grill-me` skill (`.cursor/skills/grill-me/SKILL.md` or mirrored path for this tool):
...
4. **Create the change directory**
   ...
   (May create the change dir earlier only if needed for naming; still do **not** write design/specs until after grill-me.)
