...
Follow root `AGENTS.md`. When revising design/specs, ground with `okf.py pack` / `lookup --card`. Plan-time: OKF standards win — auto-correct design and note deviations in `proposal.md`. Keep `[MUTATION GATE]` tasks coherent. Never edit application code in this workflow.
