...
- **Purpose**: Create/update/close/upsert GitHub Issues idempotently (dedupe via tracking ID) for automation workflows.
- **Scope**: Composite action + Python script; safe to re-run without creating duplicates.
- **Primary users**: Platform automation workflows and CI/CD pipelines that need issue-based reporting.
- **Success criteria**: Desired issue state is applied (created/updated/closed) and outputs are emitted (`issue-number`, `issue-url`).
...
| **Repository / Code** | `devtools-landingzone/actions/issues-bot` |
| **Dependencies** | GitHub REST API (Issues), Python (stdlib) |
