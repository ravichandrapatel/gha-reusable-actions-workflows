...
def fetch_ref(ref: str) -> None:
    """INTENT: Fetch a single ref from origin (ref-agnostic). Parent syntax (sha^) fetches the tip then deepens.
    ...
    if len(root) == 40 and root == ZERO_SHA:
        return
    depth = "2" if suffix else "1"
    run_git_cmd(["git", "fetch", "origin", root, f"--depth={depth}", "--no-tags"], ignore_error=True)
    if suffix:
...
def resolve_ref(ref: str) -> str:
    ...
    if is_zero_sha(ref):
        return ref
    root, suffix = split_rev_suffix(ref)
    # Try ref as-is (local branch, tag, SHA, sha^), then origin/root[+suffix]
    ...
    if suffix:
        for tip in (root, f"origin/{root}"):
            try:
                tip_sha = run_git_cmd(["git", "rev-parse", "--verify", tip])
                return run_git_cmd(["git", "rev-parse", "--verify", f"{tip_sha}{suffix}"])
            except subprocess.CalledProcessError:
                continue
    raise ValueError(
...
def get_changed_files_with_status(
    ...
    Zero-SHA base: list all files in source commit. Resolves refs after fetch so manual runs work (no local branch needed).
    INPUT: base_ref, source_ref. OUTPUT: List[(status, path)]. SIDE_EFFECTS: subprocess."""
    # [T-04] Fetch both refs (no origin/ prefix)
    fetch_ref(base_ref)
    fetch_ref(source_ref)

    if is_zero_sha(base_ref):
        # [T-05] New branch: list all files in source (resolve so origin/source works when local branch missing)
        source_rev = resolve_ref(source_ref)
        out = run_git_cmd(["git", "ls-tree", "-r", "--name-only", source_rev])
        if not out:
            return []
        return [("A", Path(p).as_posix()) for p in out.splitlines() if p.strip()]

    # [T-06] Resolve refs so "main" and "feature.test" work after fetch (origin/main, origin/feature.test)
    base_rev = resolve_ref(base_ref)
    source_rev = resolve_ref(source_ref)

    # Same revision (e.g. both "main" when run on main): no changes, skip git diff to avoid revision/merge-base errors
    if base_rev == source_rev:
        return []

    # Two-dot diff (base -> source): no merge base needed, works in shallow clones and manual runs.
    out = run_git_cmd(
        ["git", "diff", "--name-status", base_rev, source_rev],
        ignore_error=True,
    )
    if not out:
        return []

    pairs = _parse_diff_status_lines(out)
    _log(f"[DBG-002] Found {len(pairs)} changed file(s) with status.")
    return pairs


def _parse_diff_status_lines(out: str) -> list[tuple[str, str]]:
    """INTENT: Parse git diff --name-status output (newline-separated); normalize CRLF/CR, strip excess.
    INPUT: out (str). OUTPUT: List[(status, path)] with status in A/M/D and path POSIX. SIDE_EFFECTS: None."""
    ...
    for line in out.replace("\r\n", "\n").replace("\r", "\n").split("\n"):
        ...
        if status_map not in "AMD":
            status_map = "M"
        pairs.append((status_map, Path(path).as_posix()))
    return pairs
