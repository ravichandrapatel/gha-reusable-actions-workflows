...
* **Removed kernel/profiles:** Unused Profile schema/RBAC template — persona is AGENTS.md + IDE agents. Kernel no longer walks profiles markdown; dropped `Profile` from type seed.
* **Copilot custom agent:** `.github/agents/aegis-okf.agent.md` — binding from Cursor `.cursor/rules/aegis-okf.mdc`.
* **Copilot skills + prompts:** Mirrored Cursor skills → `.github/skills/*/SKILL.md`; Cursor commands → `.github/prompts/*.prompt.md` (same seven workflows).
* **Aegis Cursor skills:** Added `aegis-discover`, `okf-pack`, `mutation-gate`, `okf-writeback`, `okf-maintain`, `aegis-review` (+ matching `/` commands). Wired into AGENTS lifecycle and Extending Aegis replicate list.
* **DNA restore v5.1.0:** `AGENTS.md` is protocol DNA again (persona, BINDING discovery/Rule #1/lifecycle). Deploy/copy-paste lives in `vault/concepts/extending-aegis.md` only.
* **GitHub Copilot binding:** Added `.github/copilot-instructions.md` + `.github/instructions/{aegis-okf,okf-brain,gha-spvs}.instructions.md`.
* **Package rewrite v5.0.0:** Lean DNA; Rule #1 = pack; `standards/okf-house-schema.md`; slimmed meta; refreshed system inventory.
* **OpenSpec removal (OKF-only):** Deleted `openspec/`, `/opsx:*`, openspec skills; capability probe Brain-gated.
